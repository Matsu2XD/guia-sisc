...
function useLocalStorageChecklist() {
  const [completed, setCompleted] = useState<Record<string, boolean>>(() => {
    try {
      const saved = localStorage.getItem('sisc-checklist')
  ...
  const toggle = useCallback(
    (id: string) => {
      setCompleted((prev) => {
        ...
        try {
          localStorage.setItem('sisc-checklist', JSON.stringify(next))
        } catch {}
        return next
      })
    },
  ...
  const reset = useCallback(() => {
    setCompleted({})
    try {
      localStorage.setItem('sisc-checklist', JSON.stringify({}))
    } catch {}
  }, [])
